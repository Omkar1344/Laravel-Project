<?php

namespace App\Console\Commands;

use Illuminate\Console\Command;
use Illuminate\Support\Facades\Storage;

class CreateDefaultImage extends Command
{
    protected $signature = 'app:create-default-image';
    protected $description = 'Create a default product image';

    public function handle()
    {
        // Create the storage directory if it doesn't exist
        Storage::disk('public')->makeDirectory('products');
        
        // This is a minimal valid JPEG image (1x1 pixel, white)
        $base64Image = '';
        
        // Decode and save the image
        $imageData = base64_decode($base64Image);
        Storage::disk('public')->put('default-product.jpg', $imageData);
        
        $this->info('Default product image created successfully!');
    }
} 